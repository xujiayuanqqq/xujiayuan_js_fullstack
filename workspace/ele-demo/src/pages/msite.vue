      ...
      <div class="swiper-container" v-if="foodTypes.length">
        <div class="swiper-wrapper">
          <div class="swiper-slide food_types_container" v-for="(item, index) in foodTypes" :key="index">
            <router-link v-for="foodItem in item" :key="foodItem.id" class="link_to_food" :to="{path: '/food', query: { geohash, title: foodItem.title, restraurant_category_id: foodItem.link }}">
              <figure>
...
export default {
    ...
    // /city/25?a=1
    // this.cityid = this.$route.params.cityid; params在 url的parse里面
    async beforeMount() {
    ...
    async mounted () {
      ...
      // vnode => dom diff 差
      // mvvm 数据改变 到 界面更新 异步操作 vnode dom node
      // vue 等待这个过程结束？
